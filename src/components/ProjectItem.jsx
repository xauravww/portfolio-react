...
const ProjectItem = ({ img, description, techStacks, url }) => {
  ...
  const live = url.live
  return (
    <div className="project-item-container w-full relative z-[7] bg-white rounded-md shadow-md relative h-[50vw] lg:h-[20vw]" onMouseEnter={() => sethoverItem(true)} onMouseLeave={() => sethoverItem(false)}>
      <img className="project-item-img rounded-t-md h-full w-full object-cover" src={img} alt="" />
      <div 
        className={`wrapper-description-and-btns absolute inset-x-0 bottom-0 bg-slate-100 opacity-0 scale-100 transition-all duration-300 ease-in-out transform-gpu ${hoverItem ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}
      >
   {hoverItem && (
