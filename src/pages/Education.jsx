...
function Education({ containerId }) {
  ...
  return (
    <div className="flex  flex-col items-center justify-center min-h-screen bg-[var(--bg-dark)] relative" id={containerId}>
      ...
      <div className="border-double border-4 border-[var(--border-color)] mt-8 max-w-[75vw] md:mt-16 mx-6 md:mx-8 p-4 text-[var(--text-light)] grid grid-cols-1 md:grid-cols-2 gap-4 z-[3] bg-[var(--bg-dark)] rounded-xl">
        <div className="m-2 md:m-4">
          {classArray.map((item) => (
            <div
              key={item}
              className={`py-2 md:py-4 px-4 md:px-12 text-xl md:text-2xl font-bold mt-2 md:mt-4 cursor-pointer ${showItem === item ? "border-l-4 border-[var(--accent-blue)]" : ""}`}
              onClick={handleClick}
            >
              {item}
            </div>
